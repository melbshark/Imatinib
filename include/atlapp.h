...
public:
...
#else // !(_ATL_VER >= 0x0700)
		::EnterCriticalSection(&_pModule->m_csStaticDataInit);
		return S_OK;
#endif // !(_ATL_VER >= 0x0700)
	}
	...
	void Unlock()
	{
		...
		m_cslock.Unlock();
#else // !(_ATL_VER >= 0x0700)
		::LeaveCriticalSection(&_pModule->m_csStaticDataInit);
#endif // !(_ATL_VER >= 0x0700)
	}
};
...
public:
	...
	HRESULT Lock()
	{
		...
		return m_cslock.Lock();
#else // !(_ATL_VER >= 0x0700)
		::EnterCriticalSection(&_pModule->m_csWindowCreate);
		return S_OK;
#endif // !(_ATL_VER >= 0x0700)
	}
	...
	void Unlock()
	{
		...
		m_cslock.Unlock();
#else // !(_ATL_VER >= 0x0700)
		::LeaveCriticalSection(&_pModule->m_csWindowCreate);
#endif // !(_ATL_VER >= 0x0700)
	}
};
